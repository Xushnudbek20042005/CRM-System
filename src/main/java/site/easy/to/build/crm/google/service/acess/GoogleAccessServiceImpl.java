...
//                throw new RuntimeException(e);
    ...
    public void verifyAccessAndHandleRevokedToken(OAuthUser oAuthUser, User user, List<String> scopesToCheck) throws IOException {
        try {
            ...
            // TO'G'RILANGAN XAVFSIZ VARIANT:
            if (oAuthUser == null) {
                // Agar foydalanuvchi bazada hali yo'q bo'lsa, xato berish o'rniga jarayonni to'xtatamiz
                // yoki foydalanuvchini qayta login sahifasiga yo'naltiruvchi maxsus exception otamiz
                throw new IllegalStateException("Foydalanuvchi tizimda ro'yxatdan o'tmagan. Iltimos, avval login qiling.");
            }

// Obyekt borligi aniq bo'lgandan keyingina uning ichidagi tokenni tekshiramiz:
            String refreshToken = oAuthUser.getRefreshToken();
            if (refreshToken == null || refreshToken.isEmpty()) {
                throw new IllegalStateException("Google Refresh Token topilmadi. Qaytadan kirib ruxsat bering.");
            }
